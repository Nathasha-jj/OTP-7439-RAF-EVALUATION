 ...
 */
    (email, file, record, search, runtime) => 
    {
        ...
        const execute = (scriptContext) => 
        {
            ...
            if(unitsRemaining > 0)
            {
                try
                {
                    ...
                    let searchResultSet = createSearch();
                    ...
                    searchResultSet.each(function(searchResult)
                    {
                        ...
                        csvContent = getDetails(searchResult);
                        return true;
                    });
                    ...
                    let csvFile = filesave(fileName, csvContent);
        ...
        function sendEmail(csvFile)
        {
            email.send(
            {
                author: -5,
                recipients: 1659,
                subject: "Closed Sales Orders List ",
                body: "The sales orders which are created on or before thirty days are closed and the details are provided in the attachemnt",
                attachments: [csvFile]
            });
        }
        ...
        function getDetails(searchResult)
        {
            let csvContent;
            ...
            let totalAmount = searchResult.getValue('amount');
            log.debug("Total Amount", totalAmount);
            csvContent += soId + ',' + documentNumber+ ',' + customerName + ',' + totalAmount + '\n'
            return csvContent;
        }
        return {execute}
